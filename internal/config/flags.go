...
func (c *Config) RegisterServeFlags(cmd *cobra.Command) {
	...
	fs.StringVar(&c.Auth.File, FlagAuthFile, c.Auth.File, "Path to a file containing newline-separated scrypted auth keys")
	fs.StringVar(&c.Auth.RemoteFile, FlagAuthRemoteFile, c.Auth.RemoteFile, "Path to a file containing newline-separated scrypted auth keys for remote uploads")
	...
	fs.BoolVar(&c.S3.ForcePathStyle, FlagS3ForcePathStyle, c.S3.ForcePathStyle, "Force path-style addressing for S3 (e.g. https://s3.amazonaws.com/linx/example.txt)")
	...
	fs.DurationVar(&c.Auth.CookieExpiry.Duration, FlagAuthCookieExpiry, c.Auth.CookieExpiry.Duration, "Expiration time for access key cookies in seconds (set 0 to use session cookies)")
